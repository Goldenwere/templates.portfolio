...
import { fetchAndReturnText } from './fetch'
...
/**
 * Retrieves a project from either the store or the 
 * @returns the loaded project data
 */
export const getProject = async (store: AppStoreDefinition, id: string) => {
  return !!store.projects[id]
    ? store.projects[id]
    : fetchAndReturnProject(`/content/projects/${id}.md`)
}
